...
/**
 * This is where all video info come from
 */
public abstract class CameraActivity extends AppCompatActivity
  ...
  protected void onCreate(final Bundle savedInstanceState) {
    ...
    /****Modif****/
    pixelVal = findViewById(R.id.pixelVal);
    categorieVal = findViewById(R.id.categorieVal);
    /*************/
  ...
  /**
   * This method draw the map and set the value of Pixel and Distance
   * @param img_array
   * @param imageSizeX
   * @param imageSizeY
   */
  ...
  protected void showResultsInTexture(float[] img_array, int imageSizeX, int imageSizeY) {
    ...
    if(textureView.isAvailable()) {
      ...
      //Draw Cross in the center of the screen
      int centerX = width / 2;
      int centerY = height / 2;
      int crossSize = Math.min(width, height) / 20;

      for (int i = centerX - crossSize; i <= centerX + crossSize; i++) {
        bitmap.setPixel(i, centerY, Color.RED);
      }

      for (int j = centerY - crossSize; j <= centerY + crossSize; j++) {
        bitmap.setPixel(centerX, j, Color.RED);
      }

      //Draw Square in the center of the screen
      int squareSize = Math.min(width, height) / 8;

      for (int i = centerX - squareSize; i <= centerX + squareSize; i++) {
        for (int j = centerY - squareSize; j <= centerY + squareSize; j++) {
          if (i == centerX - squareSize || i == centerX + squareSize || j == centerY - squareSize || j == centerY + squareSize) {
            bitmap.setPixel(i, j, Color.RED);
          }
        }
      }

      int minX = centerX - squareSize;
      int maxX = centerX + squareSize;
      int minY = centerY - squareSize;
      int maxY = centerY + squareSize;

      // Calculate the average of the gray level values of the pixels in the red square
      int sum = 0;
      int count = 0;
      int dist = 0;
      int average = 0;
      ...
      //We separate our average value into three categories
      average = sum /count;
      if(average > 90){
        dist = 1;
      }
      ...
      String s_dist = "" + dist;
      String s_average = "" + average;

      //We set the value of the TextView in tfe_ic_layout_bottom_sheet.xml
      pixelVal.setText(s_average);
      categorieVal.setText(s_dist);
